function [post nlZ dnlZ] = infPEP(hyp, mean, cov, lik, x, y)
...
if ~strcmp(likstr,'likGauss')               % NOTE: no explicit call to likGauss
  error('Inference with infPEP only possible with Gaussian likelihood.');
end
cov1 = cov{1}; if isa(cov1, 'function_handle'), cov1 = func2str(cov1); end
if ~strcmp(cov1,'covPEP'); error('Only covPEP supported.'), end    % check cov
...
g_sn2 = alpha*(diagK - sum(V.*V,1)') + sn2;  % g+sn2=alpha*(diag(K)+sn2-diag(Q))
g  = g_sn2 - sn2;
...
if nargout>1                                % do we want the marginal likelihood
  nlZ = sum(log(diag(Lu))) + (sum(log(g_sn2)) + n*log(2*pi) + r'*r - be'*be...
      + (1-alpha)/alpha * sum(log(g_sn2/sn2)) )/2;
  if nargout>2                                         % do we want derivatives?
    ...
    for i = 1:numel(hyp.cov)
      ...
      R = 2*dKui-dKuui*B; v = alpha*(ddiagKi-sum(R.*B,1)'); % diag part of cov deriv
      ddiag_temp = alpha*ddiagKi + (1-alpha)*sum(R.*B,1)';
      dnlZ.cov(i) = (ddiag_temp'*(1./g_sn2)+w'*(dKuui*w-2*(dKui*al))-al'*(v.*al)...
         - sum(W.*W,1)*v - sum(sum((R*W').*(B*W'))) ...
         + (1-alpha)/alpha * v'*(1./g_sn2) )/2;
    end
    diag_dK = (1+(1-alpha)/alpha)./g_sn2 - sum(W.*W,1)' - al.*al;                  % diag(dnlZ/dK)
    dnlZ.lik = sn2*sum(diag_dK) - n*(1-alpha)/alpha;
    ...
    if isfield(hyp,'xu')                 % derivatives w.r.t. inducing points xu
      ...
      v = diag_dK;        % BdK = B * ( diag(diag(dnlZ/dK)) - dnlZ/dK )
      BdK = alpha*B.*repmat(v',nu,1) + BWt*W + (B*al)*al' - B.*repmat(1./g_sn2',nu,1);
