function [post nlZ dnlZ] = infVFE(hyp, mean, cov, lik, x, y)
...
if ~strcmp(likstr,'likGauss')               % NOTE: no explicit call to likGauss
  error('Inference with infVFE only possible with Gaussian likelihood.');
end
cov1 = cov{1}; if isa(cov1, 'function_handle'), cov1 = func2str(cov1); end
if ~strcmp(cov1,'covVFE'); error('Only covVFE supported.'), end    % check cov
...
g_sn2 = sn2*ones(n, 1);          % g + sn2 = diag(K) + sn2 - diag(Q)
g = diagK - sum(V.*V,1)';          % g = diag(K) - diag(Q)
...
if nargout>1                                % do we want the marginal likelihood
  ...
  if nargout>2                                         % do we want derivatives?
    ...
    for i = 1:numel(hyp.cov)
      ...
      dnlZ.cov(i) = (sum(sum(R.*B, 1))/sn2+w'*(dKuui*w-2*(dKui*al)) ...
        - sum(sum((R*W').*(B*W'))) + sum(v)/sn2)/2;
    end
    diag_dK = 1/sn2 - sum(W.*W,1)' - al.*al;                  % diag(dnlZ/dK)
    dnlZ.lik = sn2*sum(diag_dK) - sum(g)/sn2;
    ...
    dnlZ.lik = dnlZ.lik + (w'*dKuui*w - sum(sum((R*W').*BWt)) + sum(v)/sn2 )/2; 
