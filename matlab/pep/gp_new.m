...
if strcmp(cstr,'covPEP') && isfield(hyp,'xu'), cov{3} = hyp.xu; end %use hyp.xu
if isempty(inf)                                   % set default inference method
  if strcmp(cstr,'covFITC'), inf = {@infFITC};
  elseif strcmp(cstr,'covVFE'), inf = {@infVFE}; 
  else
      if strcmp(cstr,'covPEP'), inf = {@infPEP}; else inf = {@infExact}; end
  end
end
...
if strcmp(cstr,'covPEP')                              % only infPEP are possible
  if isempty(strfind(istr,'infPEP')==1)
    error('Only infPEP are possible inference algorithms')
  end
end                            % only one possible class of inference algorithms
if strcmp(cstr,'covVFE')                              % only infVFE are possible
  if isempty(strfind(istr,'infVFE')==1)
    error('Only infVFE are possible inference algorithms')
  end
end                            % only one possible class of inference algorithms
...
if nargin==7                                     % if no test cases are provided
  ...
  while nact<ns               % process minibatches of test cases to save memory
    ...
    if strcmp(cstr,'covFITC') || strcmp(cstr,'covPEP')       % cross-covariances
